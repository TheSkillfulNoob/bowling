...
def update_data_to_gsheet(date_str, location_str, games):
    ...
    date_parsed = pd.to_datetime(date_str, dayfirst=True).date().strftime("%Y/%m/%d")
    new_rows = [
...
if len(filtered) >= 5:
    ...
    with tab_ocr:
        ...
        if not uploaded:
            ...
            # 2a) Split into frames and show them for visual inspection
            from result_ocr.preprocess import to_gray, remove_red_circles
            from result_ocr.segment   import crop_row, split_frames

            # crop + clean the whole row
            bgr_row   = crop_row(bgr)  # I suspected I missed something here?
            clean_row = remove_red_circles(bgr_row)

            # split and convert to gray
            frames = split_frames(clean_row)
            gray_frames = [to_gray(remove_red_circles(f)) for f in frames]

            # display in a 10-column layout
            cols = st.columns(10)
            for i, gf in enumerate(gray_frames):
                with cols[i]:
                    st.image(gf, use_column_width=True, clamp=True)
                    st.caption(f"F{i+1}")
